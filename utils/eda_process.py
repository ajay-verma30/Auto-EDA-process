...
#     pdf = PDF()
#     pdf.add_font("DejaVu", "", os.path.join(font_dir, "DejaVuSans.ttf"), uni=True)  # Regular
#     pdf.add_font("DejaVu", "B", os.path.join(font_dir, "DejaVuSans-Bold.ttf"), uni=True)  # Bold
#     pdf.add_font("DejaVu", "I", os.path.join(font_dir, "DejaVuSans-Oblique.ttf"), uni=True)  # Italic
...
def generate_plot(df_to_plot, col, plot_type):
    ...
    if pd.api.types.is_numeric_dtype(df_to_plot[col]):
        ...
        if plot_type == 'Bar Chart':
            value_counts = df_to_plot[col].value_counts().head(10) # Limit to top 10 for clarity
            bars = ax.bar(range(len(value_counts)), value_counts.values, color=colors[:len(value_counts)])
            ax.set_xticks(range(len(value_counts)))
            ax.set_xticklabels(value_counts.index, rotation=45, ha='right')
            ax.set_title(f"Distribution of {col}", fontsize=16, fontweight='bold', pad=20)
...
class PDF(FPDF):
    ...
    def chapter_body(self, body):
        ...
        # Use a more robust way to handle multi-line text
...
def create_pdf_report(df, overall_summary, plot_data_for_report):
    ...
    # Sanitizing summary before writing to handle potential encoding issues
    ...
    # The fix is here: remove the .encode() method
...
def eda_section(df):
    ...
    # Sanitize dataframe for PDF report to avoid encoding issues
    def sanitize_dataframe_for_pdf(df):
        df_sanitized = df.copy()
        for col in df_sanitized.columns:
            if pd.api.types.is_string_dtype(df_sanitized[col]):
    ...
    with st.sidebar:
        ...
        # Set a reasonable sample size limit for large datasets
        max_rows = len(df)
    ...
    col1, col2 = st.columns([2, 1])
    with col1:
        # Filter out columns with all unique values (like IDs)
        valid_cols = [col for col in df.columns if df[col].nunique() < len(df)]
        selected_col = st.selectbox(
            "🔍 Select column for analysis",
            valid_cols,
    ...
    # AI insights for univariate plot
